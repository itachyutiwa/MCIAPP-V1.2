...
class Prestation:
    ...
    def conso_autorisees(self,file_benef,file_cotisation,file_conso):
        ...
        consommation = self.database(file_benef,file_cotisation,file_conso)["db"]["consommation"]
        df1 = benef["Num Police"].dropna().apply(lambda x: str(x).replace(" ","")).drop_duplicates().tolist()
        df2 = cotisation["Num Police"].dropna().apply(lambda x: str(int(x)).replace(" ","")).drop_duplicates().tolist()
        df3 = consommation["Num Police"].dropna().apply(lambda x: str(x).replace(" ","")).drop_duplicates().tolist()
        ...
        result_3 = [id for id in df3 if id not in df1 and id not in df2]
        return {
            "Consommations autorisées":set(result_1+result_2+result_3)
        }
    ...
    def periode_couverture(self,file_benef,file_cotisation,file_conso,date_fin):
        ...
        return {
            "2.Période de couverture": filtrage
        }
    ...
    def condition_age(self,file_benef,file_cotisation,file_conso,date_fin="31/12/2022"):
        ...
        ENFANT_SUP_25_CONSO = sans_doublons_conso[cond1]["NUM_POLICE"].apply(lambda x: str(int(x))).tolist()

        cond2 = sans_doublons_conso['Matricule Beneficiaire'].isin(ADULTES_SUP_60["Matricule"].unique())
        ADULTES_SUP_60_CONSO = sans_doublons_conso[cond2]["Num Police"].apply(lambda x: str(int(x))).tolist()
        
        return {
            "3.Conditions d’âge": set(ENFANT_SUP_25_CONSO + ADULTES_SUP_60_CONSO)
        }
    ...
    def double_conso(self,file_benef,file_cotisation,file_conso):
        consommation = self.database(file_benef,file_cotisation,file_conso)["db"]["consommation"]
        dble_conso = set(consommation[consommation.duplicated(keep=False)]["Num Police"].apply(lambda x: str(int(x))).tolist())
        return {
                "4.Double consommations":dble_conso
        }
    ...
    def depense_imortantes(self,file_benef,file_cotisation,file_conso):
        ...
        conso_sorted = consommation.sort_values(by='MONTANT_PAYE', ascending=False).drop_duplicates(subset=['MONTMontant PayeNT_PAYE','Num Police'], keep='first')
        ...
        return {
            "6.Dépenses importantes":n_premiers_NUM_POLICE
        }
